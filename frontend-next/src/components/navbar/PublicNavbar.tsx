...
export function PublicNavbar() {
  ...
  const AuthSection = ({ onClose }: { onClose?: () => void }) => {
    ...
    if (isAuthenticated && user) {
      ...
      const displayName =
        user.firstName || user.fullName || t("navbar.profile");
          ...
          >
            {user.avatar ? (
              <Image
                ...
                width={28}
                height={28}
                className="w-7 h-7 rounded-full object-cover ring-2 ring-brand-blue/30"
              />
            ) : (
    ...
    <div className="sticky top-4 z-50 px-4 md:px-10">
      ...
      <nav
        style={{
          ...
          boxShadow:
            "0 4px 24px rgba(0,0,0,0.06), 0 1px 4px rgba(0,0,0,0.04), inset 0 1px 0 rgba(255,255,255,0.75)",
