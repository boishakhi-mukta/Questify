...
export default function Navbar() {
  ...
  const AuthSection = ({ onClose }: { onClose?: () => void }) => {
    if (isLoading) return <div className="w-20 h-8 rounded bg-brand-border/50 animate-pulse" />;
    ...
    if (isAuthenticated && user) {
      ...
      return (
        <div className="flex items-center gap-3">
          ...
          >
            {user.avatar ? (
              <Image src={user.avatar} alt={displayName} width={32} height={32} className="w-8 h-8 rounded-full object-cover" />
            ) : (
